...
| --- | --- | --- | --- | --- |
| Devices | ✅ | `CaptureDevice`, `MediaType`, `AuthorizationStatus`, `CaptureDeviceDetails` | `Device.swift` | Enumeration, lookup, active format/frame-duration inspection, configuration lock, flash/torch mode. |
...
| Photo output | ✅ | `PhotoOutput`, `PhotoOutputInfo`, `PhotoCaptureResult` | `PhotoOutput.swift` | Capability inspection, key macOS setters, and delegate-to-closure photo-capture bridge. |
...
| Examples / tests | ✅ | `examples/01`-`10`, `tests/*.rs` | n/a | Every logical area has at least one numbered example and one integration test. |

## Deferred / skipped Apple SDK rows

The table above reflects the macOS capture surface covered by `avcapture` 0.2.0. The following SDK slices remain intentionally deferred and are called out here instead of being omitted silently:
...
2. ⏭️ `AVCaptureDevice` focus / exposure / white-balance / zoom / depth / reaction-effect tuning APIs — mostly iOS-only camera-control surfaces that are not part of the requested capture-session abstraction.
3. ⏭️ `AVCapturePhotoOutput` Live Photo, depth / portrait / semantic-segmentation, bracketed capture, Apple ProRAW, and readiness-coordinator types — unavailable on macOS or substantially larger than the `v0.2.0` photo-capture subset.
