...
fn main() {
    ...
    println!("cargo:rustc-link-lib=static=AVCaptureBridge");
    ...
    if let Ok(output) = Command::new("xcode-select").arg("-p").output() {
        if output.status.success() {
            ...
            let sdk_framework_path = format!(
                "{xcode_path}/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/System/Library/Frameworks"
            );
            println!("cargo:rustc-link-search=native={swift_lib_path}");
            println!("cargo:rustc-link-search=framework={sdk_framework_path}");
